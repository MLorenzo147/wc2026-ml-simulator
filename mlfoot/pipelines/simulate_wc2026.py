...
def simulate_wc2026(
    model_path: Path = config.MODELS_DIR / "champion_xgboost.pkl",
    models_dir: Path = config.MODELS_DIR,
    feature_columns: Path | None = None,
    label_classes: Path | None = None,
    historical_matches: Path = config.INTL_RESULTS,
    fifa_rankings: Path | None = None,
    team_features: Path | None = None,
    groups_csv: Path = config.WC_GROUPS,
    schedule_csv: Path = config.WC_SCHEDULE,
    output: Path = config.SIM_OUTPUT,
    ...
    feature_columns_path = feature_columns or (models_dir / "feature_columns.json")
    label_classes_path = label_classes or (models_dir / "label_classes.json")
    feature_columns_list = load_json(feature_columns_path)
    label_classes_list = load_json(label_classes_path)
    ...
    rankings_df = None
    if fifa_rankings:
        rankings_df = pd.read_csv(fifa_rankings)
        rankings_df = normalize_teams(rankings_df, ["team"])

    team_features_df = None
    if team_features:
        team_features_df = pd.read_csv(team_features)
    ...
    schedule_df = load_schedule(schedule_csv)
    ...
    logger.info("Saved simulation results to %s", output)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--n-simulations", type=int, default=config.DEFAULT_N_SIMULATIONS)
    parser.add_argument("--output", default=str(config.SIM_OUTPUT))
    ...
    simulate_wc2026(
        ...
        feature_columns=Path(args.feature_columns) if args.feature_columns else None,
        label_classes=Path(args.label_classes) if args.label_classes else None,
        historical_matches=Path(args.historical_matches),
        fifa_rankings=Path(args.fifa_rankings) if args.fifa_rankings else None,
        team_features=Path(args.team_features) if args.team_features else None,
